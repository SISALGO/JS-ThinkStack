function getMaxLikeStudent(nowData, map) {
  let maxLikeStudent = [];
  let maxCnt = 0;
  for (let row = 0; row < map.length; row++) {
    for (let col = 0; col < map.length; col++) {
      ...
      let cnt = 0;
      if (row - 1 >= 0) {
        if (nowData.indexOf(map[row - 1][col]) !== -1) {
          cnt++;
        }
      }
      ...
      if (col - 1 >= 0) {
        if (nowData.indexOf(map[row][col - 1]) !== -1) {
          cnt++;
        }
      }
...
const setStudent = (map, maxEmptySpace, nowStudent) => {
  // 정렬되었으므로 가장 먼저 나오는 것이 최적 값, map에 학생 배치
...
const solution = (N, data) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const maxLikeStudent = getMaxLikeStudent(data[i], map);

    //비어있는 칸 가장 많은 것 구하기
    const maxEmptySpace = getMaxEmptySpace(map, maxLikeStudent);

    //학생 배치
    map = setStudent(map, maxEmptySpace, nowStudent);
  }
